...
```bash
todo-together$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
todo-together$ sam local start-api
todo-together$ curl http://localhost:3000/
```
...
```bash
todo-together$ sam logs -n HelloWorldFunction --stack-name todo-together --tail
```
...
```bash
todo-together$ pip install pytest pytest-mock --user
todo-together$ python -m pytest tests/ -v
```
